...
class AMTDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        #print(f"data_home received: {data_home}")


        """
        # Debug-Ausgabe
        print(f"Received data_home: {data_home}")

        if data_home is None:
            raise ValueError("data_home must not be None!")

        # Sicherstellen, dass der Pfad existiert
        if not os.path.exists(data_home):
            raise ValueError(f"Data home path does not exist: {data_home}")
    ...
    def set_merged_train_data_info(self) -> None:
        ...
        # Hier Übergabeparameter self.data_home übergeben
        self.train_data_info = create_merged_train_dataset_info(self.preset_multi, self.data_home)
        print(
